...
let web3 = new Web3();
let txnData = web3.eth.abi.encodeFunctionSignature({
  name: 'createAgent',
  type: 'function',
  inputs: [{type: 'uint256',name:""}, {type: 'address',name:""}, {type: 'string',name:""}, {type: 'uint256[]', name:""}]
}) 
+ web3.eth.abi.encodeParameters(['uint256', 'address', 'string', 'uint256[]'], 
        ['2345675643', '0x56D9e2Ce76F9E97337938112230B1Ca3506A858f', 'Hello world', ['9696','2343']]).slice(2,);
...
describe("Testing basic functionanlity of bot deployment detection bot", () => {
  ...
  describe("Tests #1", () => {
    ...
    it("Notices agent deployment", async () => {
      const mockTxFunc = new TestTransactionEvent().setFrom(NETHERMIND_ADDR).setTo(FORTA_ADDR).setData(txnData);
